...
class SoccerBall(): 
    ...
    def move(self, players): 
        ...
        self.vx = self.constrain(self.vx, -4, 4)
        self.vy = self.constrain(self.vy, -4, 4)
        ...
        for a in players: 
            ...
            if(dist < self.radius + 25): 
                ...
                self.vx = 4 * np.cos(np.pi + bounce_direction)
                self.vy = 4 * np.sin(np.pi + bounce_direction)
...
soccer_ball = SoccerBall(radius=150)
