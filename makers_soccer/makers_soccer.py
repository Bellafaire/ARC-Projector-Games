...
class SoccerBall(): 
    def __init__(self, radius=50, bounds=(0,0, 1920,1080), initial_position=(1920/2,1080/2), initial_velocitiy=None):
        ...
        self.sprite = pygame.image.load("gear.png")
    ...
    def move(self, players): 
        ...
        self.vx = self.constrain(self.vx, -20, 20)
        self.vy = self.constrain(self.vy, -20, 20)
        ...
        for a in players: 
            ...
            if(dist < self.radius): 
                ...
                self.vx = 1.1 * magnitude * np.cos(np.pi + bounce_direction)
                self.vy = 1.1 * magnitude * np.sin(np.pi + bounce_direction)
                
                self.radial_velocity = max(self.radial_velocity * 1.1, 1)
...
soccer_ball = SoccerBall(radius=100)
